...
export function RoomBookingView({ events, config, roomName }: RoomBookingViewProps) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", padding: "2rem" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
        ...
        <DisplayClock format={config.clockFormat} className="text-3xl font-light opacity-70" />
      </div>
      ...
      {currentEvent && !currentEvent.isAllDay && (
        <div style={{ marginTop: "1.5rem", padding: "1.5rem", borderRadius: "0.75rem", backgroundColor: "var(--display-busy)15" }}>
          ...
          <div style={{ marginTop: "0.5rem", fontSize: "0.875rem", opacity: 0.6 }}>
            {new Date(currentEvent.startTime).toLocaleTimeString("de-DE", { hour: "2-digit", minute: "2-digit" })} - {new Date(currentEvent.endTime).toLocaleTimeString("de-DE", { hour: "2-digit", minute: "2-digit" })}
          </div>
        </div>
      )}
      {nextEvents.length > 0 && (
        <div style={{ marginTop: "auto", paddingTop: "1.5rem" }}>
          ...
          {nextEvents.map((event) => (
            <div key={event.id} style={{ display: "flex", alignItems: "center", gap: "1rem", padding: "0.75rem 0", borderTop: "1px solid var(--display-muted)22" }}>
              <div style={{ fontSize: "0.875rem", fontWeight: 500, opacity: 0.7, width: "5rem", flexShrink: 0 }}>{new Date(event.startTime).toLocaleTimeString("de-DE", { hour: "2-digit", minute: "2-digit" })}</div>
